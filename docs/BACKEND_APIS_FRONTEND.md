...
El registro envía la notificación. Un email no verificado recibe `403` al intentar login. Al verificarse, la cuenta se vincula automáticamente al Customer sin cuenta que tenga el mismo email normalizado. `/api/user` y `/api/me/context` incluyen `email_verified` y `email_verified_at`.
...
`POST /api/auth/forgot-password` recibe `{ "email": "persona@email.com" }` y siempre responde `200` con el mismo mensaje, exista o no la cuenta. Esto evita revelar emails registrados. Tiene rate limit de 6 solicitudes por minuto.

El email contiene una URL al frontend: `FRONTEND_URL/reset-password?token=...&email=...`. Configurar `FRONTEND_URL` en cada entorno (por ejemplo, `https://app.midominio.com`).

`POST /api/auth/reset-password` recibe:
...
```json
{
  ...
  "confirm_password": "nuevaClaveSegura"
}
```

El token vence en 60 minutos y es de un solo uso. Si venció o no coincide, responde `422`. Al restablecerse la clave se revocan **todos** los tokens Sanctum del usuario; el frontend debe redirigir a login y no conservar el Bearer anterior.
