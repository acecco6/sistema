...
final class ClubCustomerApiTest extends TestCase
{
    ...
    {
        $user = User::factory()->create(); $role = Role::factory()->withPermission($permission)->create();
    ...
    {
        $club = Club::factory()->create(); $staff = $this->staff($club, 'customer.create');
        $created = $this->actingAs($staff, 'sanctum')->postJson("/api/clubs/{$club->id}/customers", [
            'name' => 'Juan Pérez', 'email' => 'juan@test.com', 'phone' => '1111', 'notes' => 'Prefiere turno tarde',
    ...
    {
        $clubA = Club::factory()->create(); $clubB = Club::factory()->create(); $account = User::factory()->create();
    ...
    {
        $clubA = Club::factory()->create(); $clubB = Club::factory()->create(); $staff = $this->staff($clubB, 'customer.view');
