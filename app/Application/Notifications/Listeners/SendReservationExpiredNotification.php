...
final class SendReservationExpiredNotification implements ShouldQueue
{
    ...
    public function handle(ReservationExpired  $event): void
    {
        ...
        $emailLog = $this->emailLogs->save(
            new EmailLog(
                ...
                notificationType: ReservationExpiredNotification::class,
                template: null,
                payload: [
        ...
        try {
            Notification::route('mail', $toEmail)->notify(new ReservationExpiredNotification($dto));
            $emailLog->markSent();
